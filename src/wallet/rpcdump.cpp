...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"moonroomcashprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"moonroomcashprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Moonroomcash address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid Moonroomcash spending key
        if (fImportZKeys) {
            try {
                CZCSpendingKey spendingkey(vstr[0]);
                libmoonroomcash::SpendingKey key = spendingkey.Get();
                libmoonroomcash::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a Moonroomcash style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"moonroomcashaddress\"\n"
            "\nReveals the private key corresponding to 'moonroomcashaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"moonroomcashaddress\"   (string, required) The moonroomcash address for the private key\n"
            "\nResult:\n"
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"folder/filename\"    (string, required) The filename, saved in folder set by moonroomcashd -exportdir option or specified in input. Exportdir option is primary. If no folder is provided, filename will be saved in folder from which the command is executed.\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Moonroomcash %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    if (fDumpZKeys) {
        ...
        for (auto addr : addresses ) {
            libmoonroomcash::SpendingKey key;
