...
{
public:
    libmoonroomcash::PaymentAddress address;
    ...
    CNoteData(libmoonroomcash::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libmoonroomcash::PaymentAddress a, uint256 n) :
...
{
    JSOutPoint jsop;
    libmoonroomcash::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libmoonroomcash::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    bool AddZKey(const libmoonroomcash::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libmoonroomcash::SpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libmoonroomcash::PaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libmoonroomcash::PaymentAddress &addr, const libmoonroomcash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libmoonroomcash::PaymentAddress &address, const libmoonroomcash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);

    //! Adds a viewing key to the store, and saves it to disk.
    bool AddViewingKey(const libmoonroomcash::ViewingKey &vk);
    bool RemoveViewingKey(const libmoonroomcash::ViewingKey &vk);
    //! Adds a viewing key to the store, without saving it to disk (used by LoadWallet)
    bool LoadViewingKey(const libmoonroomcash::ViewingKey &dest);
    ...
    boost::optional<uint256> GetNoteNullifier(
        ...
        const libmoonroomcash::PaymentAddress& address,
