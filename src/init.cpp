...
ZCJoinSplit* pmoonroomcashParams = NULL;
...
void Shutdown()
{
    ...
    RenameThread("moonroomcash-shutoff");
    ...
    pwalletMain = NULL;
#endif
    delete pmoonroomcashParams;
    pmoonroomcashParams = NULL;
...
bool static Bind(const CService &addr, unsigned int flags) {
    if (!(flags & BF_EXPLICIT) && IsLimited(addr))
        return false;
    std::string strError;
    if (!BindListenPort(addr, strError, (flags & BF_WHITMRCIST) != 0)) {
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "moonroomcash.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "moonroomcashd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16225, 26225));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16224, 26224));
...
{
    RenameThread("moonroomcash-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
static void ZC_LoadParams()
{
    ...
    if (!(boost::filesystem::exists(pk_path) && boost::filesystem::exists(vk_path))) {
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Moonroomcash network parameters in the following directory:\n"
              "%s\n"
              "Please run 'moonroomcash-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pmoonroomcashParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Moonroomcash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Moonroomcash is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Moonroomcash is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Moonroomcash is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Moonroomcash circuit parameters
    ZC_LoadParams();
    ...
    if (fListen) {
        if (mapArgs.count("-bind") || mapArgs.count("-whitebind")) {
            ...
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs["-whitebind"]) {
                ...
                fBound |= Bind(addrBind, (BF_EXPLICIT | BF_REPORT_ERROR | BF_WHITMRCIST));
            }
        }
        else {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Moonroomcash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Moonroomcash to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Moonroomcash was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Moonroomcash with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Moonroomcash was not built with wallet support. Set -mineraddress, or rebuild Moonroomcash with wallet support."));
    }
 #endif // !ENABLE_WALLET
