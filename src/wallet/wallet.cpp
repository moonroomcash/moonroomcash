...
bool CWallet::AddZKey(const libmoonroomcash::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libmoonroomcash::PaymentAddress &address,
                                    const libmoonroomcash::ReceivingKey &rk,
...
bool CWallet::LoadCryptedZKey(const libmoonroomcash::PaymentAddress &addr, const libmoonroomcash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libmoonroomcash::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libmoonroomcash::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libmoonroomcash::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libmoonroomcash::ViewingKey &vk)
{
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pmoonroomcashParams, wtxItem.second.joinSplitPubKey);
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pmoonroomcashParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent, bool ignoreUnspendable)
{
    ...
    libmoonroomcash::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pmoonroomcashParams, wtx.joinSplitPubKey);
            try {
