...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libmoonroomcash::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libmoonroomcash::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libmoonroomcash::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libmoonroomcash::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libmoonroomcash::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libmoonroomcash::PaymentAddress &address, libmoonroomcash::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libmoonroomcash::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libmoonroomcash::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
