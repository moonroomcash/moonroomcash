...
#if defined(NDEBUG)
# error "Moonroomcash cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Moonroomcash Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state,
                      libmoonroomcash::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pmoonroomcashParams, verifier, tx.joinSplitPubKey)) {
...
void ThreadScriptCheck() {
    RenameThread("moonroomcash-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    auto verifier = libmoonroomcash::ProofVerifier::Strict();
    auto disabledVerifier = libmoonroomcash::ProofVerifier::Disabled();
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In Moonroomcash this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Moonroomcash protocol spec
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libmoonroomcash::ProofVerifier::Disabled();
