...
CWalletTx GetValidReceive(const libmoonroomcash::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libmoonroomcash::Note GetNote(const libmoonroomcash::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libmoonroomcash::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libmoonroomcash::SpendingKey::random();
    auto sk2 = libmoonroomcash::SpendingKey::random();
