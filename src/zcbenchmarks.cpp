...
#include "moonroomcash/Moonroomcash.h"
#include "moonroomcash/IncrementalMerkleTree.hpp"
...
double benchmark_create_joinsplit()
{
    ...
    assert(jsdesc.Verify(*pmoonroomcashParams, verifier, pubKeyHash));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 pubKeyHash;
    auto verifier = libmoonroomcash::ProofVerifier::Strict();
    joinsplit.Verify(*pmoonroomcashParams, verifier, pubKeyHash);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    for (int i = 0; i < nAddrs; i++) {
        auto sk = libmoonroomcash::SpendingKey::random();
        wallet.AddSpendingKey(sk);
    }

    auto sk = libmoonroomcash::SpendingKey::random();
    auto tx = GetValidReceive(*pmoonroomcashParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pmoonroomcashParams, sk, 10, true);
        auto note = GetNote(*pmoonroomcashParams, sk, wtx, 0, 1);
