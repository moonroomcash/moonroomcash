...
#else
    const char* pszModule = "Moonroomcash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Moonroomcash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Moonroomcash
    // Mac: ~/Library/Application Support/Moonroomcash
    // Unix: ~/.moonroomcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Moonroomcash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "moonroomcash.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        throw missing_moonroomcash_conf();
    ...
    {
        // Don't overwrite existing settings so command line settings override moonroomcash.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "moonroomcashd.pid"));
...
{
    return "\n" +
           FormatParagraph(strprintf(_("In order to ensure you are adequately protecting your privacy when using Moonroomcash, please see <%s>."),
