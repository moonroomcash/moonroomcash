#!/usr/bin/env python2
#
# Execute all of the automated tests related to Moonroomcash.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/moonroomcash/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/moonroomcash/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/moonroomcashd')
    ret &= test_rpath_runpath('src/moonroomcash-cli')
    ret &= test_rpath_runpath('src/moonroomcash-gtest')
    ret &= test_rpath_runpath('src/moonroomcash-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/moonroomcash/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/moonroomcashd')
    ret &= test_fortify_source('src/moonroomcash-cli')
    ret &= test_fortify_source('src/moonroomcash-gtest')
    ret &= test_fortify_source('src/moonroomcash-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/moonroomcash/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/moonroomcash-gtest')],
