...
#include "moonroomcash/Proof.hpp"
...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libmoonroomcash's job to ensure the
    ...
    {
        JSDescription jsdesc(*pmoonroomcashParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pmoonroomcashParams, verifier, pubKeyHash));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pmoonroomcashParams, verifier, pubKeyHash));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(*pmoonroomcashParams, pubKeyHash, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pmoonroomcashParams, pubKeyHash, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pmoonroomcashParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pmoonroomcashParams, verifier, pubKeyHash));
    }
}
