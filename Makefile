...
CONFIG_CLEAN_FILES = libmoonroomcashconsensus.pc src/test/buildenv.py \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libmoonroomcashconsensus.pc.in \
...
abs_builddir = /root/ws/moonroomcash
abs_srcdir = /root/ws/moonroomcash
...
pkgconfig_DATA = libmoonroomcashconsensus.pc
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  ...
  moonroomcash-gtest.info moonroomcash-gtest_filtered.info moonroomcash-gtest_coverage.info

dist_bin_SCRIPTS = zcutil/fetch-params.sh
dist_noinst_SCRIPTS = autogen.sh zcutil/build-debian-package.sh zcutil/build.sh
EXTRA_DIST = $(top_srcdir)/share/genbuild.sh qa/pull-tester/rpc-tests.sh qa/pull-tester/run-bitcoin-cli qa/rpc-tests qa/moonroomcash $(DIST_DOCS) $(BIN_CHECKS)
...
libmoonroomcashconsensus.pc: $(top_builddir)/config.status $(srcdir)/libmoonroomcashconsensus.pc.in
...
#moonroomcash-gtest.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ moonroomcash-gtest_check
#	$(LCOV) -c -d $(abs_builddir)/src -t moonroomcash-gtest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#moonroomcash-gtest_filtered.info: moonroomcash-gtest.info
#	$(LCOV) -r $< "/usr/include/*" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/*.h" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/boost/*" \
#                      "$(abs_builddir)/depends/x86_64-unknown-linux-gnu/include/gmock/*" \
...
#moonroomcash-gtest_coverage.info: baseline_filtered_combined.info moonroomcash-gtest_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a moonroomcash-gtest_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_bitcoin_filtered.info moonroomcash-gtest_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoin_filtered.info -a moonroomcash-gtest_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitcoin.coverage/.dirstamp:  test_bitcoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#moonroomcash-gtest.coverage/.dirstamp:  moonroomcash-gtest_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#cov-moonroomcash: moonroomcash-gtest.coverage/.dirstamp

#total.coverage/.dirstamp: total_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@

#cov: test_bitcoin.coverage/.dirstamp cov-moonroomcash total.coverage/.dirstamp

install-exec-hook:
	mv $(DESTDIR)$(bindir)/fetch-params.sh $(DESTDIR)$(bindir)/moonroomcash-fetch-params

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_bitcoin.coverage/ moonroomcash-gtest.coverage/ total.coverage/
