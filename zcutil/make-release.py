...
def main(args=sys.argv[1:]):
    """
    Perform the final Moonroomcash release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libmoonroomcashconsensus.pc',
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'Moonroomcash Company'
    sh_log(
        'debchange',
...
def patch_README(release, releaseprev):
    with PathPatcher('README.md') as (inf, outf):
        ...
        assert firstline == 'Moonroomcash {}\n'.format(releaseprev.novtext), \
            repr(firstline)

        outf.write('Moonroomcash {}\n'.format(release.novtext))
        outf.write(inf.read())
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "moonroomcash-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "moonroomcash-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './moonroomcash-make-release.log'
    ...
    logging.info('moonroomcash make-release.py debug log: %r', logname)
